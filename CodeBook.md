...
The data set that this code book pertains to is located in the `data.txt` file of this repository.
...
4. The variable names were replaced with descriptive variable names (e.g. `tBodyAcc-mean()-X` was expanded to `timeDomainBodyAccelerometerMeanX`), using the following set of rules:
	...
	- `Acc`, `Gyro`, `Mag`, `Freq`, `mean`, `angle`, `gravity`, `tBody` and `std` were replaced with `Accelerometer`, `Gyroscope`, `Magnitude`, `Frequency`, `Mean`, `Angle`, `Gravity`, `TimeBody` and `STD` respectively.
